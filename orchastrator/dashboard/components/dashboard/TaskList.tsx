    ...
    >
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          ...
          {isCompleted && task.result?.project_path && (
            <div className="flex items-center gap-1.5 mt-2.5 text-[10px] text-cyan-500 font-bold tracking-wide">
              <svg className="h-3 w-3 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z" />
              </svg>
              <span>View generated project &rarr;</span>
            </div>
          )}

          {task.status === 'failed' && task.error && (
            <p className="text-[10px] text-red-500/70 mt-1.5 truncate" title={task.error}>{task.error}</p>
          )}
        </div>
      </div>
    </Link>
